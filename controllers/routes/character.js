...
const getCharacterIDs = async (req, res) => {
  ...
  if (cacheContent && duration && totalCharacters) {
    ...
    try {
      ...
      util
        .getRemainingCharacters(count, total, charactersArr)
        .then((data) => {
          util.heroesCache.setKey(HEROES_IDS_KEY, data);
          util.heroesCache.setKey(TOTAL_CHARACTER_KEY, data.length);
          util.heroesCache.setKey(
            DURATION_KEY,
  ...
  try {
    ...
    if (character) {
      ...
      _axios
        ...
        .then(
          (result) => {
            character = new Character(
              result.data.data.results[0].id,
              result.data.data.results[0].name,
              result.data.data.results[0].description
            );
        ...
        .catch((err) => {
          res.status(404).send(new ErrorHandler(404, err.response.data.status || "Please check"));
        });
    }
  } catch (error) {
    console.log(error);
    res.status(500).end(new ErrorHandler(500, error.message || "Server error, Please report to Developer."));
  }
};
